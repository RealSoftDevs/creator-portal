// app/components/dashboard/Sidebar.tsx - Update the profile button
'use client';
...
interface SidebarProps {
  ...
  totalClicks: number;
  isPremium: boolean;
  onProfileSettings?: () => void; // Keep for compatibility but not used
  onLogout: () => void;
  isMobileDrawer?: boolean;
  onClose?: () => void;
}

export default function Sidebar({ activeTab, setActiveTab, linksCount, productsCount, totalClicks, isPremium, onLogout, isMobileDrawer, onClose }: SidebarProps) {
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {isPremium && (
          <div className="mb-6 p-3 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl flex items-center gap-3">
            ...
            <div><p className="text-white font-semibold text-sm">Premium User</p><p className="text-white/80 text-xs">Unlimited everything</p></div>
          </div>
        )}
        ...
        <div className="mt-6 pt-6 border-t" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-4 py-2 rounded-lg bg-white/10">
              <div className="flex items-center gap-2 text-white/70"><Link2 className="w-4 h-4" /><span className="text-sm">Total Links</span></div><span className="font-semibold text-white">{linksCount}</span>
            </div>
            <div className="flex items-center justify-between px-4 py-2 rounded-lg bg-white/10">
              <div className="flex items-center gap-2 text-white/70"><Package className="w-4 h-4" /><span className="text-sm">Total Products</span></div><span className="font-semibold text-white">{productsCount}</span>
            </div>
            <div className="flex items-center justify-between px-4 py-2 rounded-lg bg-white/10">
              <div className="flex items-center gap-2 text-white/70"><TrendingUp className="w-4 h-4" /><span className="text-sm">Total Clicks</span></div><span className="font-semibold text-white">{totalClicks}</span>
            </div>
          </div>
        </div>

        <div className="mt-6 pt-6 border-t" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
          {/* Link to profile page instead of modal */}
  ...
  if (isMobileDrawer) {
    return (
      <div className="mobile-sidebar fixed top-0 right-0 bottom-0 w-80 bg-gray-900 z-50 shadow-2xl transform transition-transform duration-300 overflow-y-auto">
        <div className="sticky top-0 bg-gray-900 p-4 border-b border-gray-800 flex justify-end">
